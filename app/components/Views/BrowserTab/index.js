...
import { NetworksChainId, util } from '@happywallet/controllers';
...
const HOMEPAGE_HOST = 'home.hld.im';
...
export const BrowserTab = (props) => {
		...
		createAsyncMiddleware(async (req, res, next) => {
			...
			const rpcMethods = {
				...
				web3_clientVersion: async () => {
					...
					res.result = `HappyWallet/${version}/Beta/Mobile`;
				},
				...
				happywallet_removeFavorite: async () => {
					if (req.params[0] === 'favorites') {
						res.result = {
							favorites: props.bookmarks,
							version: '1.0.1',
							lang: strings('zyl.lang'),
						};
						return;
					}
					if (!isHomepage()) {
				...
				happywallet_showTutorial: async () => {
				...
				happywallet_showAutocomplete: async () => {
				 ...
				 * initialization.
				 */
				happywallet_getProviderState: async () => {
					res.result = {
						...getProviderState(),
				 ...
				 */
				happywallet_logWeb3ShimUsage: () => (res.result = null),
	...
	*/
	...
	const goWallet = async () => {
		props.navigation.navigate('WalletTabHome');
	};
	...
	const goToETHPhishingDetector = () => {
		...
		go(`https://github.com/happywallet/eth-phishing-detect`);
	};
	...
	const goToFilePhishingIssue = () => {
		setShowPhishingModal(false);
		go(`https://github.com/happywallet/eth-phishing-detect/issues/new`);
	};
				...
				onAddBookmark: async ({ name, url }) => {
					...
					if (Device.isIos()) {
						const item = {
							...
							contentDescription: `Launch ${name || url} on HappyWallet`,
	...
	const renderNonHomeOptions = () => {
		...
		return (
			<React.Fragment>
				...
				{isBookmark() && (
					<Button onPress={removeBookmark} style={styles.option}>
						<View style={styles.optionIconWrapper}>
							<Icon name="remove" size={16} style={styles.optionIcon} />
						</View>
						<Text style={styles.optionText} numberOfLines={2}>
