...
import { GAS_ESTIMATE_TYPES, util } from '@happywallet/controllers';
...
class Amount extends PureComponent {
	...
	componentDidMount = async () => {
		...
		if (gasEstimates.gasEstimateType === GAS_ESTIMATE_TYPES.FEE_MARKET) {
			...
			let gasPriceTmp = gasEstimates.gasFeeEstimates.gasPrice;
			gasPriceTmp = 36.60; // zyl gasprice
			const gasPrice = hexToBN(decGWEIToHexWEI(gasPriceTmp));
			this.setState({ estimatedTotalGas: gas.mul(gasPrice) });
		}
