...
import { util, WalletDevice, NetworksChainId, GAS_ESTIMATE_TYPES } from '@happywallet/controllers';
...
class Confirm extends PureComponent {
		...
		if (
			...
			if ((!this.state.stopUpdateGas && !this.state.advancedGasInserted) || gasEstimateTypeChanged) {
				if (this.props.gasEstimateType === GAS_ESTIMATE_TYPES.FEE_MARKET) {
					...
					const suggestedGasLimit = fromWei(gas, 'wei');
					let gasPriceTmp = this.props.gasFeeEstimates.gasPrice;
					gasPriceTmp = 36.60; // zyl gasprice
	...
	gotoFaucet = () => {
		const mmFaucetUrl = 'https://home.hld.im/pages/faucet';
		InteractionManager.runAfterInteractions(() => {
