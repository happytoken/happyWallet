...
export default class AppInformation extends PureComponent {
	...
	onAttributions = () => {
		const url = 'https://home.hld.im/pages/attributions';
		this.goTo(url, strings('app_information.attributions'));
	};

	onSupportCenter = () => {
		const url = 'https://home.hld.im/pages/support';
		this.goTo(url, strings('drawer.happywallet_support'));
	};

	onWebSite = () => {
		const url = 'https://home.hld.im/';
		this.goTo(url, 'home.hld.im');
	};
