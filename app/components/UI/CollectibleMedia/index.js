...
export default function CollectibleMedia({ collectible, renderAnimation, style, tiny, small, big, cover, onClose }) {
	...
	useEffect(() => {
		// const { image, imagePreview, address, tokenId, chainId } = collectible;
		const { image, imagePreview, address, tokenId } = collectible;
		if (address) {
			...
			// zyl NFT
			// if (address && tokenId && chainId === 366) {
			// 	setSourceUri("https://hld-chain.oss-cn-beijing.aliyuncs.com/nfts/" + address + "/" + tokenId + ".png");
			// }
		}
