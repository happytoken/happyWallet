...
export class NetworkList extends PureComponent {
	...
	networkElement = (selected, onPress, name, color, i, network) => (
		...
		>
			...
			<RemoteImage
				fadeIn
				source={
					name === 'Binance Smart Chain Mainnet' ? bnbLogo : name === 'Heco Chain Mainnet' ? htLogo : name === 'Okex Chain Mainnet' ? oktLogo : hldLogo
				}
				style={[styles.icon]}
			/>
	...
	renderRpcNetworks = () => {
		...
		return frequentRpcList.map(({ nickname, rpcUrl }, i) => {
			...
			return this.networkElement(selected, this.onSetRpcTarget, name, color || colors[i], i, rpcUrl);
		});
	};
