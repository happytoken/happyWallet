...
class Tokens extends PureComponent {
	...
	hldPrices = {};
	componentDidMount() {
		AsyncStorage.getItem('hldPrices').then((val) => {
			this.hldPrices = val;
		})
	}
	...
	renderItem = (asset) => {
		...
		if (this.hldPrices) {
			try{
				var str = JSON.stringify(this.hldPrices).replace(new RegExp("\\\\\"", "gm"), "\"");
				var json = JSON.parse(str.substring(1, str.length - 1));
				if (asset.isETH && asset.symbol === 'HLD') {
					secondaryBalance =  "$ "+ (parseFloat(mainBalance.split(' ')[0]) * parseFloat(json["0x846091c5db00cf432ca0f64a9110447b53060734"])).toFixed(4);
				} else if (itemAddress && json.hasOwnProperty(itemAddress.toLowerCase())) {
					secondaryBalance = "$ " + (parseFloat(mainBalance.split(' ')[0]) * parseFloat(json[itemAddress.toLowerCase()])).toFixed(4);
				} else {
					secondaryBalance = "$ 0.0000";
				}
			}catch(e){
			}
		}
		...
		return (
			...
			>
				...
				<View style={styles.symbolText}>
					<Text>{mainBalance.split(' ')[1]}</Text>
				</View>
				<View style={styles.balances} testID={'balance'}>
					<Text style={styles.balance}>{mainBalance.split(' ')[0]}</Text>
					{true && secondaryBalance ? (
