...
import { WalletDevice, util, GAS_ESTIMATE_TYPES } from '@happywallet/controllers/';
...
function SwapsQuotesView({
	...
	const slippageRatio = useMemo(
		() =>
			parseFloat(
				new BigNumber(selectedQuote?.priceSlippage?.ratio || 0, 10).minus(1, 10).times(100, 10).toFixed(2),
				10
			),
		[selectedQuote]
	);
	...
	const [isPriceDifferenceModalVisible, togglePriceDifferenceModal, , hidePriceDifferenceModal] =
	...
	const openLinkAboutGas = () =>
		Linking.openURL('https://home.hld.im/pages/gas');
	...
	return (
		<ScreenView contentContainerStyle={styles.screen} keyboardShouldPersistTaps="handled">
			...
			<InfoModal
				...
				title={strings('swaps.happywallet_swap_fee')}
				body={
