...
class AssetOverview extends PureComponent {
	...
	copyAccountToClipboard = async () => {
		...
		this.props.showAlert({
			...
			data: { msg: strings('asset_overview.token_copied_to_clipboard') },
		});
	...
	goToBrowserUrlCross = () =>
		this.props.navigation.navigate('BrowserTabHome', {
			screen: 'BrowserView',
			params: {
				newTabUrl: 'https://cross.hld.im/#/cross?chainFrom=0&chainTo=366',
			},
		});

	goToBrowserUrlSimple(url, title) {
	...
	renderLogo = () => {
		...
		if (!iconUrl && address) {
			iconUrlTmp = 'https://hld-chain.oss-cn-beijing.aliyuncs.com/tokens/' + address + '.png';
		}

		return iconUrlTmp ? <AssetIcon logo={iconUrlTmp} /> : <Identicon address={address} />;
	};
	...
	renderWarning = () => {
		...
		const supportArticleUrl =
			'https://home.hld.im/pages/balance';
		return (
	...
	render() {
		...
		let showTokenAddress = true;
		if ((chainId === '1' && symbol === 'ETH') || (chainId === '56' && symbol === 'BNB') || (chainId === '128' && symbol === 'HT') || (chainId === '66' && symbol === 'OKT') || (chainId === '366' && symbol === 'HLD')) {
			showTokenAddress = false;
		} // zyl
		let showTokenSwap = false;
		if (chainId === '366' && symbol === 'USDT') {
			showTokenSwap = true;
		}

		return (
			<View style={styles.wrapper} testID={'token-asset-overview'}>
				<TouchableOpacity onPress={() => this.goToBrowserUrlSimple("https://home.hld.im/pages/token?lang=" + (strings('zyl.lang')) + "&chain=" + chainId + "&address=" + address, symbol)}>
				<View style={styles.assetLogo}>{this.renderLogo()}</View>
				</TouchableOpacity>
				{showTokenAddress && (<TouchableOpacity style={styles.addressWrapper} onPress={this.copyAccountToClipboard}>
					<Text>{strings('asset_overview.copyTokenAddress')}</Text><EthereumAddress onPress={this.copyAccountToClipboard} address={address} style={styles.address} type={'short'} />
				</TouchableOpacity>)}
				{false && (<View style={styles.balance}>
					{balanceError ? (
						this.renderWarning()
					) : (
						<>
				...
				{!balanceError && (
					<View style={styles.actions}>
						...
						/>
						{false && AppConstants.SWAPS.ACTIVE && (
							<AssetSwapButton
						...
						{false && (<AssetActionButton
							testID={'token-swap-button'}
							icon="swap"
							onPress={this.goToBrowserUrlCross}
							label={strings('asset_overview.cross_button')}
						/>)}
					</View>
				)}
			</View>
