...
prebuild(){
	# Import provider
	cp node_modules/@happywallet/mobile-provider/dist/index.js app/core/InpageBridgeWeb3.js
...
prebuild_android(){
	...
	yes | cp -rf ./app/fonts/happywallet.ttf ./android/app/src/main/assets/fonts/happywallet.ttf
...
buildIosSimulatorE2E(){
	prebuild_ios
	cd ios && xcodebuild -workspace HappyWallet.xcworkspace -scheme HappyWallet -configuration Debug -sdk iphonesimulator -derivedDataPath build
}
...
generateArchivePackages() {
	xcodebuild -workspace HappyWallet.xcworkspace -scheme HappyWallet -configuration Release COMIPLER_INDEX_STORE_ENABLE=NO archive -archivePath build/HappyWallet.xcarchive -destination generic/platform=ios && xcodebuild -exportArchive -archivePath build/HappyWallet.xcarchive -exportPath build/output -exportOptionsPlist HappyWallet/IosExportOpitions.plist
}
...
buildIosReleaseE2E(){
	...
	if [ "$PRE_RELEASE" = true ] ; then
		...
		cd ios && xcodebuild -workspace HappyWallet.xcworkspace -scheme HappyWallet -configuration Release -sdk iphonesimulator -derivedDataPath build
	fi
}

buildAndroidRelease(){
	if [ "$PRE_RELEASE" = false ] ; then
		adb uninstall im.hld || true
	fi
	prebuild_android
...
if [ "$MODE" == "release" ] || [ "$MODE" == "releaseE2E" ]; then
	...
	if [ -z "$happywallet_ENVIRONMENT" ]; then
		printError "Missing happywallet_ENVIRONMENT; set to 'production' for a production release, 'prerelease' for a pre-release, or 'local' otherwise"
		exit 1
	fi
fi
