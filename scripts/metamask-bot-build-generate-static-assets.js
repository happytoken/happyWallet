...
const contractMetadata = require('@happywallet/contract-metadata');
const fs = require('fs');
...
const main = async () => {
	...
	for (let i = 0; i < cmKeys.length; i++) {
		...
		if (['bnb.png', 'xgg.svg'].indexOf(token.logo) < 0) {
			continue;
		}
		...
		await fs.appendFileSync(
			...
			`\n\t${isBlacklisted ? '//' : ''}'${
				token.logo
			}': require('happywallet/node_modules/@happywallet/contract-metadata/images/${token.logo}'),`
		);
	}
	await fs.appendFileSync(imageModulesPath, '\n};\n');
