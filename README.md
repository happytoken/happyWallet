...
HappyWallet is a mobile web browser that provides easy access to websites that use the [Ethereum](https://ethereum.org/) blockchain.

For up to the minute news, follow our [Twitter](https://twitter.com/happywallet) or [Medium](https://medium.com/happywallet) pages.

To learn how to develop HappyWallet-compatible applications, visit our [Developer Docs](https://docs.happywallet.io).
...
-   _HappyWallet Only:_ Rename the `.*.env.example` files (remove the `.example`) in the root of the project and fill in the appropriate values for each key. Get the values from another HappyWallet Mobile developer.

-   Clone this repo and install our dependencies:

```bash
git clone ...
cd happywallet-mobile
yarn install # this will run a lengthy postinstall flow
cd ios && pod install && cd .. # install pods for iOS
```

-   _Non-HappyWallet Only:_ In the project root folder run
...
-   _Non-HappyWallet Only:_ Create an account and generate your own API key at [Infura](https://infura.io) in order to connect to main and test nets. Fill `MM_INFURA_PROJECT_ID` in `.js.env`. (App will run without it, but will not be able to connect to actual network.)
...
-   Install the Android SDK, via [Android Studio](https://developer.android.com/studio).
    -   _HappyWallet Only:_ To create production builds, you need to install Google Play Licensing Library via the SDK Manager in Android Studio.
